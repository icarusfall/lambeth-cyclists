...
# Authentication
# ---------------------------------------------------------------------------
# Comma-separated so each caller can hold its own key: the portal and the
# Vercel chat app currently use different ones, and a single shared secret
# would mean rotating either breaks the other.
...
class BearerAuthMiddleware:
    ...
    The Anthropic MCP connector forwards the `authorization_token` from the
    caller's mcp_servers config as a bearer token. Without this check the
    server hands every database in DATABASES — committee action items and
    councillor research included — to anyone who knows the Railway URL.
    """
...
if __name__ == "__main__":
    ...
    if transport == "stdio":
        ...
        logger.info("Starting Lambeth Cyclists MCP server — transport=stdio")
        mcp.run(transport="stdio")
    else:
        if not MCP_API_KEYS:
            sys.exit(
                "MCP_API_KEY is not set. This server exposes read access to every "
                "Notion database in DATABASES, so it refuses to start unauthenticated "
                "over HTTP. Set MCP_API_KEY to every key its callers send — the "
                "portal's and the Vercel chat app's — comma-separated."
            )
        ...
        logger.info(
            ...
            "(bearer auth on, %d key(s) accepted)",
            port, transport, len(MCP_API_KEYS),
        )
        uvicorn.run(
